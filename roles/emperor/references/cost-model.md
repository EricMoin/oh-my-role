...
- `maxConcurrent: 5` limits per-model concurrent sessions. Since the tree uses two model pools (Opus for emperor, deepseek-v4-pro-max for chancellor/departments, deepseek-v4-flash for jinyiwei routing), each pool has its own 5-slot semaphore. Background tasks use 4 slots (5 minus 1 reserved for sync).
...
|-------|---------|----------------|
| `hfai-anthropic/cloudsway-claude-opus-4.6-cache-1M` | Emperor | independent 5-slot |
| `hfai/deepseek-v4-pro-max` | Chancellor, Drafter, Reviewer, Finalizer, UI, Backend, Test | independent 5-slot |
| `hfai/deepseek-v4-flash` | Jinyiwei | independent 5-slot |
