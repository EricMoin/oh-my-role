...
Record each finding in research_evidence as {source, claim}. The source includes
the source type and path/URL/version; claim states exactly what it establishes.
The notation below describes citation text, not a different report format.
...
**Action:** Prefer the documentation. Report the discrepancy in the execution report in the structured `research_evidence` array. Include the source and the behavior it establishes.
...
**Action:** Explicitly flag the claim as `assumption — not verified` in the execution report. Do NOT present it as fact. Do NOT fabricate citations. Record the uncertainty in incomplete_items; an assumption does not satisfy required research. Never fabricate a citation.
