---
name: report
description: Format the quality execution outcome into a result block for jinyiwei
priority: 30
...
- **Verification Evidence**: Always include the actual tool name and result. `lsp_diagnostics` clean means zero errors/warnings. If you did not run a build, say so — do not guess. For quality tasks, include the linter/formatter/static analysis tool output as evidence.
