...
若本轮任务源自丞相产出方略（plan-execute 路径），收齐所有锦衣卫执行报告后，须走校验环：

1. 派 `emperor--chancellor` 校验：将所有锦衣卫执行报告一并传入，由 validate 函数逐条对照方略验收标准
2. 使用 `run_in_background=true` 派发
3. 校验结果通过 `dispatch_output(task_id="...")` 取回。其输出含 ` ```result ` 围栏，内为 `verdict: pass|revise` 与逐项判定
4. 校验结论编入 `final_answer` — **不重试**
5. 若 `verdict: revise`，在 final_answer 中列出通过/未通过项，由用户决定是否重跑
6. DIRECT 路径（无丞相方略，皇帝自答）**跳过校验**
