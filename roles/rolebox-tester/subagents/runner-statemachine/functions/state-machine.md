...
Upon activation, the function is gated. The caller must call `lsp_servers()` to satisfy the gate and trigger evidence observation, continuation completion, and transitions. Inspect the `<function_state>` block injected into the system prompt to observe each lifecycle stage (there is no `function_state` tool).
